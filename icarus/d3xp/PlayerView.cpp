...
/*
	...
	float scale = ( 256 + ( ( gameLocal.random.RandomInt()&63 ) - 32 ) ) / 256.0f;
	blob->w = 1280 * g_blobSize.GetFloat() * scale;
	blob->h = 960 * g_blobSize.GetFloat() * scale;
	float s1 = 0.0f;
	float t1 = 0.0f;
	float s2 = 1.0f;
	float t2 = 1.0f;
...
void idPlayerView::SingleView( idUserInterface *hud, const renderView_t *view ) {
	...
	if ( !pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool() ) {
		...
		if ( armorPulse > 0.0f && armorPulse < 1.0f ) {
			...
			renderSystem->DrawStretchPic( 0, 0, 1280, 960, 0, 0, 1, 1, armorMaterial );
		}
		...
		if ( alpha < 1.0f  ) {
			...
			renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, tunnelMaterial );
		}

		if ( bfgVision ) {
			renderSystem->SetColor4( 1.0f, 1.0f, 1.0f, 1.0f );
			renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, bfgMaterial );
		}
	...
	if ( g_testPostProcess.GetString()[0] ) {
		...
		if ( !mtr ) {
			...
			renderSystem->SetColor4( 1.0f, 1.0f, 1.0f, 1.0f );
			renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, mtr );
		}
	}
}
...
void FullscreenFX_Warp::HighQuality() {
	...
	for ( float i = 0; i < 360; i += STEP ) {
		...
		p.outer1.z = p.outer1.x / 1280.f;
		p.outer1.w = 1 - ( p.outer1.y / 960.f );

		p.outer2.x = center.x + x2 * radius;
		p.outer2.y = center.y + y2 * radius;
		p.outer2.z = p.outer2.x / 1280.f;
		p.outer2.w = 1 - ( p.outer2.y / 960.f );

		p.center.x = center.x;
		p.center.y = center.y;
		p.center.z = p.center.x / 1280.f;
		p.center.w = 1 - ( p.center.y / 960.f );
...
void FullscreenFX_EnviroSuit::HighQuality() {
	renderSystem->SetColor4( 1, 1, 1, 1 );
	renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, material );
}
...
void FullscreenFX_InfluenceVision::HighQuality() {
	...
	if ( player->GetInfluenceMaterial() ) {
		...
		renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 0.0f, 1.0f, 1.0f, player->GetInfluenceMaterial() );
...
void FullscreenFXManager::Blendback( float alpha ) {
	...
	if ( alpha < 1.f ) {
		...
		renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, shiftScale.y, shiftScale.x, 0.f, blendBackMaterial );
	}
}
...
void FullscreenFXManager::Process( const renderView_t *view ) {
	...
	if ( !highQualityMode ) {
		...
		renderSystem->SetColor4( 1, 1, 1, 1 );
		renderSystem->DrawStretchPic( 0.0f, 0.0f, 1280.0f, 960.0f, 0.0f, 1, 1, 0.f, blendBackMaterial );
	}
}
