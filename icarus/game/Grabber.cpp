...
void idGrabber::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteBool( grabberRotationAllowed );
}
...
void idGrabber::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadBool( grabberRotationAllowed );
}
...
void idGrabber::Initialize( void ) {
	if ( !gameLocal.isMultiplayer ) {
		...
		if ( !beam ) {
			...
			args.SetBool( "start_off", true );
			args.Set( "width", "6" );
			args.Set( "skin", "textures/materials_used_in_source/physics_manipulation_device_beam" );
...
void idGrabber::StartDrag( idEntity *grabEnt, int id ) {
	...
	holdingAF = false;
	dragFailTime = gameLocal.time;
	startDragTime = gameLocal.time;
	...
	UpdateBeams();
	// Show the beams!
	// boyette note - you probably want to put a def boolean variable here to allow hiding of the beam.
// boyette begin
	bool	grabberShowBeamFX = thePlayer->weapon.GetEntity()->spawnArgs.GetBool( "grabberShowBeamFX" );
	if ( grabberShowBeamFX == true ) {
		if ( beam ) {
			beam->Show();
		}
		if ( beamTarget ) {
...
// boyette end
	...
	//boyette replaced // grabEnt->timeGroup = TIME_GROUP2;
	grabEnt->timeGroup = TIME_GROUP1;
	...
	// boyette begin - show grabber warp fx
	bool	grabberShowWarpFX = thePlayer->weapon.GetEntity()->spawnArgs.GetBool( "grabberShowWarpFX" );
	if ( grabberShowWarpFX == true ) {
		warpId = thePlayer->playerView.AddWarp( phys->GetOrigin(), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 160, 2000 );
	}
	// boyette end - show grabber warp fx
}
...
void idGrabber::StopDrag( bool dropOnly ) {
	...
	if ( dragEnt.IsValid() ) {
		...
		if ( holdingAF ) {
			...
			af_Phys->SetTimeScaleRamp( MS2SEC(gameLocal.time) - 1.5f, MS2SEC(gameLocal.time) + 1.0f );
		}
		...
		if ( !ent->IsType( idProjectile::Type ) && ( dropOnly || drag.GetDistanceToGoal() > DRAG_FAIL_LEN ) ) {
			...
			thePlayer->StartSoundShader( declManager->FindSound( "source_default_grabber_maindrop_01" ), SND_CHANNEL_WEAPON, 0, false, NULL );
			...
			// boyette grabber shooting begin
			bool grabberShootingAllowed = thePlayer->weapon.GetEntity()->spawnArgs.GetBool( "grabberShootingAllowed" );
			if ( grabberShootingAllowed == false ) {
				ent->GetPhysics()->SetLinearVelocity( vec3_origin );
				//thePlayer->StartSoundShader( declManager->FindSound( "source_default_grabber_maindrop_01" ), SND_CHANNEL_WEAPON, 0, false, NULL ); // BOYETTE NOTE: commented this out 07 19 2016 because it doesn't make sense to have an electronic noise when holding an object with your hands
				...
				thePlayer->StartSoundShader( declManager->FindSound( "source_default_grabber_fire_release_throw_01" ), SND_CHANNEL_WEAPON, 0, false, NULL );
			}
			// boyette grabber shooting end
...
int idGrabber::Update( idPlayer *player, bool hide ) {
	...
	if ( gameLocal.isMultiplayer ) {
		...
		if ( gameLocal.time < startDragTime ) {
			allow = false;
		}
	}
	...
	if ( dragEnt.GetEntity() && allow ) {
		...
		// boyette grabber infinite hold time begin
		bool	grabberAllowInfiniteHoldTime = thePlayer->weapon.GetEntity()->spawnArgs.GetBool( "grabberAllowInfiniteHoldTime" );
		if ( gameLocal.time > endTime && grabberAllowInfiniteHoldTime == false ) {
			StopDrag( true );
			return 3;
		}
		// boyette grabber infinite hold time end
		...
		if ( g_grabberHardStop.GetBool() ) {
			...
			// boyette rotation begin - it will only do a random rotate if grabberRotationAllowed is set to true.
			//const float MAX_ROTATION_SPEED = 0.f; 
			// boyette rotation end

			idVec3	angVel = entPhys->GetAngularVelocity();
			float	rotationSpeed = angVel.LengthFast();

// boyette grabber rotate begin - if it is set to false there will be no random rotation of the grabobject
			bool	grabberRotationAllowed = thePlayer->weapon.GetEntity()->spawnArgs.GetBool( "grabberRotationAllowed" );
			if ( grabberRotationAllowed == false ) {
				angVel.NormalizeFast();
				angVel *= 0.0;
				entPhys->SetAngularVelocity( angVel );
			}
// boyette grabber rotate end
...
// boyette begin - always orient in one direction
		//idAngles ang = player->firstPersonViewAxis[0].ToAngles();
		//ang.pitch += 90.f;
		//entPhys->SetAxis( ang.ToMat3() );
// boyette end - always orient in one direction
		...
		if ( drag.GetDistanceToGoal() > DRAG_FAIL_LEN ) {
			if ( dragFailTime < (gameLocal.time - 500) ) {
				StopDrag( true );
				return 3;
			}
		} else {
