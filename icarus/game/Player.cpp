				...
				if ( !hostile_entities_present && !hostile_entities_on_board ) {
					...
					if ( src->ReadToken( &token2 ) ) {
						if ( token2 == ";" ) {
							...
							gameLocal.Warning( "playership_hire gui command needs the 1st argument.", token2 );
							return true;
						} else {
							const idDict* entity_dict_to_spawn = gameLocal.FindEntityDefDict( token2 , false );
							if ( !entity_dict_to_spawn ) {
								gameLocal.Warning( "Tried to hire unknown classname: %s", token2 );
								return true;
							}
						}
					}
					...
					if ( src->ReadToken( &token3 ) ) {
						if ( token3 == ";" ) {
							...
							gameLocal.Warning( "Tried to hire classname with non numeric currency cost argument: %s", token3 );
							return true;
						} else {
					...
					if ( src->ReadToken( &token4 ) ) {
						if ( token4 == ";" ) {
							...
							gameLocal.Warning( "Tried to hire classname with non numeric materials cost argument: %s", token4 );
							return true;
						} else {
		...
		if ( PlayerShip ) {
			if ( token.Icmp("gamble_for_something") == 0 ) { // BOYETTE NOTE TODO IMPORTANT: this is just the template - will decide what to gamble for at some future point time
				...
				if ( src->ReadToken( &token2 ) ) {
					if ( token2 == ";" ) {
						src->UnreadToken( &token2 );
						gameLocal.Warning( "gamble_for_something gui command did not specify success chance as first argument.", token2 );
						return true;
					} else if ( !idStr(token2).IsNumeric() ) {
						gameLocal.Warning( "Tried to perform gamble with non numeric success chance argument: %s", token2 );
						return true;
					} else {
			...
			if ( token.Icmp("playership_recieve_currency_for_materials") == 0 ) {
				...
				if ( src->ReadToken( &token2 ) ) {
					if ( token2 == ";" ) {
						...
						gameLocal.Warning( "playership_give_currency_for_materials specified non numeric currency cost argument: %s", token2 );
						return true;
					} else {
				...
				if ( src->ReadToken( &token3 ) ) {
					if ( token3 == ";" ) {
						...
						gameLocal.Warning( "playership_give_currency_for_materials specified non numeric materials cost argument: %s", token3 );
						return true;
					} else {
		...
		if ( weapon.GetEntity() ) {
			...
			if ( tabletfocusEnt.GetEntity() && !tabletfocusEnt.GetEntity()->IsType( idDoor::Type ) ) {
				...
				if ( ShipOnBoard ) {
					...
					weapon.GetEntity()->SetRenderEntityGuisStrings( "ship_on_board_number_of_ai_entities_detected_on_board", idStr(ShipOnBoard->AIsOnBoard.size()) );
		...
		if ( PlayerShip ) {
			...
			if ( PlayerShip->SelectedCrewMember && !PlayerShip->SelectedCrewMember->was_killed ) {
				...
				if ( PlayerShip->SelectedCrewMember->module_console_damage_def_name ) {
					...
					if ( !damageDef ) {
						gameLocal.Error( "Unknown damageDef '%s'\n", PlayerShip->SelectedCrewMember->module_console_damage_def_name );
					}
				...
				if ( PlayerShip->SelectedCrewMember->door_damage_def_name ) {
					...
					if ( !damageDef ) {
						gameLocal.Error( "Unknown damageDef '%s'\n", PlayerShip->SelectedCrewMember->door_damage_def_name );
					}
...
void idPlayer::UpdateReserveCrewMemberPortraits() {
	if ( PlayerShip ) {
		...
		if ( CaptainGui ) {
			CaptainGui->SetStateString( "playership_reserve_crew_title_text", "Reserve Crew: " + idStr(PlayerShip->reserve_Crew.size()) + "/" + idStr(PlayerShip->max_reserve_crew) );
