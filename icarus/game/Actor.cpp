...
idActor::idActor( void ) {
	...
	TransporterFXDef	= NULL;
	TransporterFX		= NULL;
	EnemyDoorToAttack	= NULL;
	prevent_actor_inflicted_same_team_damage = false;

	DefAttach			= NULL;
	// boyette space command end

// boyette space command kill count begin
	space_command_level = 0;
	killCount = 0;
// boyette space command kill count end
}
...
void idActor::Spawn( void ) {
	...
	// boyette space command begin
	EnemyDoorToAttack	= NULL;
	prevent_actor_inflicted_same_team_damage = spawnArgs.GetBool( "prevent_actor_inflicted_same_team_damage", "0" );
	// boyette space command end
}
...
void idActor::SetupHead( void ) {
	...
	//headModel = spawnArgs.GetString( "def_head", "" ); // BOYETTE NOTE: this was the original
	headModel = spawnArgs.RandomPrefix( "def_head", gameLocal.random ); // BOYETTE NOTE ENABLE THIS MAYBE
	if ( headModel[ 0 ] ) {
...
void idActor::Save( idSaveGame *savefile ) const {
	...
	// BOYETTE SAVE BEGIN
	idStr HealFXDefName = "heal_actor_emitter_def_default";
	savefile->WriteString( HealFXDefName );
	HealFX.Save( savefile );

	idStr TransporterFXDefName = "transporter_actor_emitter_def_default";
	savefile->WriteString( TransporterFXDefName );
	TransporterFX.Save( savefile );

	savefile->WriteObject( EnemyDoorToAttack );
	savefile->WriteBool( prevent_actor_inflicted_same_team_damage );

	DefAttach.Save( savefile );

	savefile->WriteInt( projectiles_to_track.size() );
	for ( i = 0; i < projectiles_to_track.size(); i++ ) {
		savefile->WriteObject( projectiles_to_track[i] );
	}

	savefile->WriteInt( space_command_level );
	savefile->WriteInt( killCount );
	// BOYETTE SAVE END
...
void idActor::Restore( idRestoreGame *savefile ) {
	...
	idStr HealFXDefName;
	savefile->ReadString( HealFXDefName );
	if ( HealFXDefName.Length() ) {
		HealFXDef = gameLocal.FindEntityDefDict( HealFXDefName );
	} else {
		HealFXDef = NULL;
	}
	HealFX.Restore( savefile );

	idStr TransporterFXDefName;
	savefile->ReadString( TransporterFXDefName );
	if ( TransporterFXDefName.Length() ) {
		TransporterFXDef = gameLocal.FindEntityDefDict( TransporterFXDefName );
	} else {
		TransporterFXDef = NULL;
	}
	TransporterFX.Restore( savefile );

	savefile->ReadObject( reinterpret_cast<idClass *&>( EnemyDoorToAttack ) );
	savefile->ReadBool( prevent_actor_inflicted_same_team_damage );

	DefAttach.Restore( savefile );

	savefile->ReadInt( num );
	projectiles_to_track.resize(num);
	for ( i = 0; i < num; i++ ) {
		savefile->ReadObject( reinterpret_cast<idClass *&>( projectiles_to_track[i] ) );
	}

	savefile->ReadInt( space_command_level );
	savefile->ReadInt( killCount );
	// BOYETTE RESTORE END
}
...
void idActor::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir, 
	...
	// boyette space command begin
	if ( prevent_actor_inflicted_same_team_damage && attacker && ( attacker->team == team ) && attacker->IsType( idActor::Type ) && !attacker->IsType( idPlayer::Type ) ) {
		// BOYETTE NOTE: there is similar logic on idProjectile to prevent bleeding particle and/or bleed texture effects.
		return;
	}
	// boyette space command end
	...
	if ( !idStr::Icmp( GetEntityDefName(), "monster_boss_d3xp_maledict" ) && 
		 (!idStr::Icmp( damageDefName, "damage_boss_monster_asteroid" ) || !idStr::Icmp( damageDefName, "damage_boss_monster_asteroid_splash" ) ) ) { // BOYETTE NOTE: was originally: (!idStr::Icmp( damageDefName, "damage_maledict_asteroid" ) || !idStr::Icmp( damageDefName, "damage_maledict_asteroid_splash" ) ) ) {
		return;
	}
	...
	if ( damage > 0 ) {
		...
		if ( health <= 0 ) {
			...
			if ( attacker && attacker->IsType(idAI::Type) ) {
				dynamic_cast<idAI*>(attacker)->killCount++;
				dynamic_cast<idAI*>(attacker)->HandleSpaceCommandLevel();
			}
			if ( gameLocal.GetLocalPlayer() && attacker && attacker == gameLocal.GetLocalPlayer() && gameLocal.GetLocalPlayer()->team == team ) {
				if ( gameLocal.GetLocalPlayer()->PlayerShip ) {
					if ( this->IsType(idAI::Type) && dynamic_cast<idAI*>( this )->ParentShip != gameLocal.GetLocalPlayer()->PlayerShip ) {
						//gameLocal.GetLocalPlayer()->PlayerShip->BecomeASpacePirateShip(); // BOYETTE NOTE: commented out 07 01 2016 
					}
				} else if ( gameLocal.GetLocalPlayer()->ShipOnBoard ) {
					// BOYETTE NOTE IMPORTANT: this would a bit risky because it would end up possibly being the same as another team - but since they other teams should not be higher than max rand this should work fine.
					//gameLocal.GetLocalPlayer()->team = idRandom::MAX_RAND + gameLocal.random.RandomInt( idRandom::MAX_RAND ); // BOYETTE NOTE: commented out 07 01 2016 
					//gameLocal.GetLocalPlayer()->spawnArgs.SetInt("team",gameLocal.GetLocalPlayer()->team); // BOYETTE NOTE: commented out 07 01 2016 
					//gameLocal.GetLocalPlayer()->DisplayNonInteractiveAlertMessage( "^1You ^1have been ^1branded a ^1space pirate.^0" ); // BOYETTE NOTE: commented out 07 01 2016 
				}
			}
			// boyette space command end
...
// boyette mode begin - this is to allow the player to be healed with negative damage defs.
		// boyette - heal begin
		health -= damage;
		health = idMath::ClampInt(-999,entity_max_health,health);
		// boyette - heal end
...
void idActor::TriggerHealFX( idStr heal_def_name ) {
	if ( !HealFX.GetEntity() || !HealFX.GetEntity()->IsActive() ) {
		if ( HealFX.GetEntity() ) {
			HealFX.GetEntity()->Event_Remove();
			HealFX = NULL;
		}
		idEntity *ent;
		const char *clsname;
		if ( !HealFX.GetEntity() ) {
			HealFXDef = gameLocal.FindEntityDefDict( heal_def_name, false ); // we probably need a spawnarg for the projectile def here. // we need to make this the sparkly particle projecile that spawns fires on impact.
			gameLocal.SpawnEntityDef( *HealFXDef, &ent, false );
			HealFX = ( idFuncEmitter * )ent; // boyette note: this is c-style cast that is converting(or designating) an idEntity into an idProjectile. A dynamic_cast or possibly a static_cast would probably work fine here instead but I haven't tried it yet.
		}
		if ( HealFX.GetEntity() ) {
			HealFX.GetEntity()->SetOrigin(GetPhysics()->GetOrigin() + idVec3(0,0,0));
			HealFX.GetEntity()->Bind(this,false);
			HealFX.GetEntity()->Event_Activate(this);
		}
	} else if ( HealFX.GetEntity() && HealFX.GetEntity()->IsActive() ) {
		if ( HealFX.GetEntity() ) {
			HealFX.GetEntity()->Event_Activate(this);
		}
	}
...
void idActor::TriggerTransporterFX( idStr transporter_def_name ) {
	if ( !TransporterFX.GetEntity() || !TransporterFX.GetEntity()->IsActive() ) {
		if ( TransporterFX.GetEntity() ) {
			TransporterFX.GetEntity()->Event_Remove();
			TransporterFX = NULL;
		}
		idEntity *ent;
		const char *clsname;
		if ( !TransporterFX.GetEntity() ) {
			TransporterFXDef = gameLocal.FindEntityDefDict( transporter_def_name, false );
			gameLocal.SpawnEntityDef( *TransporterFXDef, &ent, false );
			TransporterFX = ( idFuncEmitter * )ent;
		}
		if ( TransporterFX.GetEntity() ) {
			TransporterFX.GetEntity()->SetOrigin(GetPhysics()->GetOrigin() + idVec3(0,0,0));
			TransporterFX.GetEntity()->Bind(this,false);
			TransporterFX.GetEntity()->Event_Activate(this);
		}
	} else if ( TransporterFX.GetEntity() && TransporterFX.GetEntity()->IsActive() ) {
		if ( TransporterFX.GetEntity() ) {
			TransporterFX.GetEntity()->Event_Activate(this);
		}
	}
...
void idActor::HandleSpaceCommandLevel() {
