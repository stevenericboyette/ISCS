...
// boyette bit begin - so this won't work here(it is in precompiled.h) because all game code has to be #included in the precompiled.h - I think it's a good system but other people don't - will research more.
//#include <bitset>
// boyette bit end
...
const int MAX_WEAPONS = 64;  // - This is now completely modifiable. Got rid of the old integer bit operations which limited this to 32 weapons - in favor of std:bitset.
...
// BOYETTE BEGIN 06/09/2017
// guiOverlay - possibilities
enum {
	GUIOVERLAYISCAPTAINGUI = 0, 
	GUIOVERLAYISHAILGUI,
	GUIOVERLAYISAIDIALOGUEGUI,
	GUIOVERLAYISOTHERGUI
};
// BOYETTE END 06/09/2017
...
enum {
	GET_THE_SACRIFICE_ENDING = 0,
	GET_THE_SECRET_ENDING = 1,
	COMPLETE_THE_TUTORIAL = 2,
	WATCH_THE_INTRO = 3,
	GO_INSIDE_THE_CRYSTAL_ENTITY = 4,
	COMPLETE_THE_ARTIFICIAN_LABYRINTH = 5,
	BEAT_THE_GAME_IN_IRONMAN_MODE = 6,
};
...
public:
	int						maxHealth;
	//int						weapons;
	// to you need to create a string with the weapons bitset to a string for reading and writing and snapshotting. Then you can save/read/write the string.
	// boyette bits begin - I think this will work
	//std::bitset<MAX_WEAPONS>	willbeweapons; // this works perfectly
	//std::bitset<4>			willbeweapons(); - This will compile but it will not work because of the () - I think it expects an argument and when it doesn't have one it bombs.
	std::bitset<MAX_WEAPONS>		weapons;
	std::string						weaponsBinaryString;
	// bool		weaponsBitArray[MAX_WEAPONS]; // this is not necessary because we create a boolean array only when we need it.
	// boyette bits end - I think this will work
	int						powerups;
	int						armor;
	int						maxarmor;
	...
	// boyette infinite ammo space command begin
	bool					GiveAmmo( ammo_t type, int amount );
	// boyette infinite ammo space command end
...
// boyette space command mod begin
	// boyette mod begin
	idUserInterface			*CaptainGui;
	idUserInterface			*HailGui;
	idUserInterface			*AIDialogueGui;
	// boyette mod end
	void						UpdateCaptainMenuEveryFrame();  // boyette mod
	void						UpdateCaptainHudEveryFrame();  // boyette mod
	void						UpdateCaptainHudOnce(); // boyette mod
	int							stevetestvariable; // boyette test
	void						UpdateCaptainMenu();  // boyette mod
	void						PopulateShipList();  // boyette mod
	void						UpdateViewScreenShip();  // boyette mod
	void						UpdateStarGridShipPositions();  // boyette mod
	void						SetPlayerShipBasedOnShipSpawnArgs();  // boyette mod
	void						UpdateCrewIconPositionsOnShipDiagrams();  // boyette mod
	void						UpdateCrewIconPositionsOnHudShipDiagram();
	void						UpdateCrewMemberPortraits();  // boyette mod
	void						UpdateReserveCrewMemberPortraits();  // boyette mod
	void						CheckForPlayerShipCrewMemberNameChanges();  // boyette mod
	void						SyncUpPlayerShipNameCVars();  // boyette mod

	void						UpdateWeaponsAndTorpedosQueuesOnCaptainGui();  // boyette mod
	void						UpdateWeaponsAndTorpedosTargetingOverlays();  // boyette mod
	void						UpdateModulesPowerQueueOnCaptainGui();  // boyette mod
	void						UpdateDoorIconsOnShipDiagramsEveryFrame();  // boyette mod
	void						UpdateDoorIconsOnShipDiagramsOnce();  // boyette mod
	void						UpdateDoorIconsOnHudShipDiagramEveryFrame();  // boyette mod
	void						UpdateDoorIconsOnHudShipDiagramOnNewTarget();  // boyette mod
	void						TakeCommandOfShip(sbShip* ShipToTakeCommandOf); // boyette mod

	void						UpdateProjectileIconsOnShipDiagramsEveryFrame();  // boyette mod
	void						UpdateProjectileIconsOnHudEveryFrame();  // boyette mod

	void						ScheduleThingsToUpdateAftersbShipIsRemoved(); // boyette mod - 11 15 2014

	void						ScheduleStopWarpEffects( int ms_from_now = 0 );  // boyette mod
	void						ScheduleStopSlowMo( int ms_from_now = 0 );  // boyette mod
	void						TransitionNumBloomPassesToZero();
	void						TransitionNumBloomPassesToThirty();

	void						EnableBlur();
	void						DisableBlur();
	void						EnableDesaturate();
	void						DisableDesaturate();

	void						PostWarpThingsToDoForThePlayer();
	void						EnableWeapon();
	void						DisableWeapon();
	bool						disable_crosshairs;
	// Generic GuiOverlay functions:
	void						UpdateGuiOverlay();
	// HailGui functions.
	void						UpdateHailGui();
	void						ResetHailGuiDialogueBranchVisibilities();

	int							notificationList_counter;
	void						UpdateNotificationList(idStr notification);
	void						ResetNotificationList();
	void						DisplayNonInteractiveAlertMessage(idStr alert_message);
	void						ScheduleDisplayNonInteractiveAlertMessage(int ms_from_now, idStr alert_message);
	void						Event_DisplayNonInteractiveAlertMessage( const char *alert_message );
	int							non_interactive_alert_message_counter;
	int							last_non_interactive_alert_message_time;
	//idEntity					*TargetEntity;				// boyette mod
	idListGUI					*shipList;					// boyette list mod
	void						SetSelectedEntityFromList();			// boyette list mod // I don't think this is necessary
	idStr						stevetargetstring;				// boyette list mod

	sbShip						*PlayerShip;					// boyette - this is the player's ship.
	sbShip						*ShipOnBoard;					// boyette - this is the ship the player is currently on board.

	sbShip						*SelectedEntityInSpace;
	void						UpdateSelectedEntityInSpaceOnGuis();

	bool						currently_in_story_gui;

	sbCaptainChair*				CaptainChairSeatedIn;
	sbConsole*					ConsoleSeatedIn;

	idDict*						grabbed_reserve_crew_dict;
	idDict*						selected_reserve_crew_dict;

	idAI*						grabbed_crew_member;
	int							grabbed_crew_member_id;
	int							confirm_removal_crew_member_id;

	bool						grabbed_module_exists;
	int							grabbed_module_id;

	//void						BecomeASpacePirate();
	//void						RedeemFromBeingASpacePirate();

	// boyette mod begin
	void				SetOverlayGui(const char *guiName);
	void				CloseOverlayGui();

	void				SetOverlayCaptainGui();
	void				CloseOverlayCaptainGui();
	bool				allow_overlay_captain_gui;

	void				SetOverlayHailGui();
	void				CloseOverlayHailGui();

	void				SetOverlayAIDialogeGui();
	void				CloseOverlayAIDialogeGui();

	idUserInterface *	guiOverlay;
	bool				hud_map_visible;
	// boyette mod end

	// ai dialogue gui begin
	idAI*				DialogueAI;
	void				UpdateAIDialogueGui();
	void				ResetAIDialogueGuiBranchVisibilities();
	bool				currently_in_dialogue_with_ai;

	bool				delay_selectedship_dialogue_branch_hostile_visible; // BOYETTE NOTE: if either this is true or SelectedEntityInSpace->in_no_action_hail_mode is true, then the hostile branch will not be displayed automatically even if the ship is hostile.
	// ai dialogue gui end

	std::vector<idVec2>	stargrid_positions_visited;

	// tutorial mode begin
	bool				tutorial_mode_on;
	int					tutorial_mode_current_step;
	void				Event_ToggleTutorialMode( int on );
	void				Event_IncreaseTutorialModeStep( void );
	void				Event_GetTutorialModeStep( void );

	int					wait_impulse;
	void				Event_WaitForImpulse( int impulse_to_wait_for );
	// tutorial mode end

	// BOYETTE STEAM INTEGRATION BEGIN
	void				Event_IncreaseSteamAchievementStat( int stat_id );
	// BOYETTE STEAM INTEGRATION END
// boyette space command mod end
	...
	idEntityPtr<idLight>	playerHeadLight;
// BOYETTE SPACE COMMAND BEGIN
	idVec3					playerHeadLightOffset;
	idVec3					playerHeadLightAngleOffset;
	void					UpdatePlayerHeadLightEveryFrame();
// BOYETTE SPACE COMMAND END 
...
public:
	...
	// boyette begin
	void					RouteGuiMouseWheel( idUserInterface *gui );
	// boyette end
	void					UpdateHud( void );
	...
	idEntityPtr<idEntity>	tabletfocusEnt;
	bool					using_space_command_tablet;

	beamTarget_t			SpaceCommandTabletBeam;
	bool					space_command_tablet_beam_active;
	void					ActivateSpaceCommandTabletBeam();
	void					DeactivateSpaceCommandTabletBeam();

	void					UpdateSpaceCommandTabletEveryFrame();
	void					UpdateSpaceCommandTabletGUIOnce();
	bool					player_just_transported;
	// boyette space command tablet end

	// BOYETTE MUSIC BEGIN
	bool					music_shader_is_playing;
	idStr					currently_playing_music_shader;
	int						currently_playing_music_shader_begin_time;
	int						currently_playing_music_shader_end_time;
	void					BeginMonitoringMusic();
	void					Event_MonitorMusic();
	void					Event_CancelMonitorMusic();

	void					EndMusic();
	void					DeterminePlayerMusic();
	// BOYETTE MUSIC END

	// BOYETTE WEIRD TEXTURE THRASHING FIX BEGIN
	int						thrash_fix_frame_begin;
	int						thrash_fix_frames_to_wait;
	idVec3					before_thrash_fix_origin;
	idVec3					before_thrash_fix_view_angles;
	bool					currently_doing_weird_texture_thrash_fix;
	void					StartWeirdTextureThrashFix( const idVec3& original_org, int frames_to_wait );
	void					Event_UpdateWeirdTextureThrashFix( void );
	// BOYETTE WEIRD TEXTURE THRASHING FIX END

	// BOYETTE OVERLAY GUI SCRIPT BEGIN
	void					Event_OpenOverlayGUI( const char *overlayGUI );
	// BOYETTE OVERLAY GUI SCRIPT END

	// BOYETTE OUTRO INVENTORY ITEM SCRIPTS BEGIN
	void					Event_HasInventoryItem( const char *item_name );
...
private:
	...
	bool					prevent_weapon_firing;
	void					PreventPlayerFiringForAWhile( int prevent_firing_time = 0 );
...
//BSM: Event to remove inventory items. Useful with powercells.
	...
	void					Event_StopWarpEffects();
	void					Event_StopSlowMo();
	void					Event_TransitionNumBloomPassesToZero();
	void					Event_TransitionNumBloomPassesToThirty();
	void					Event_UpdateSomeThingsAftersbShipIsRemoved();
	void					Event_AllowPlayerFiring();
