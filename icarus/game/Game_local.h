...
#define LAGO_MATERIAL	"textures/images_used_in_source/lag_meter_placeholder"
#define LAGO_IMAGE		"textures/images_used_in_source/lag_meter_placeholder.tga"
...
#define	GAME_VERSION		"baseICARUS-1"
...
#define	GENTITYNUM_BITS			14 // BOYETTE NOTE: was originally 12 for 4096 entities
#define	MAX_GENTITIES			(1<<GENTITYNUM_BITS)
#define	ENTITYNUM_NONE			(MAX_GENTITIES-1)
#define	ENTITYNUM_WORLD			(MAX_GENTITIES-2)
#define	ENTITYNUM_MAX_NORMAL	(MAX_GENTITIES-2)

#define	MAX_STARGRID_X_POSITIONS	16
#define	MAX_STARGRID_Y_POSITIONS	12 // BOYETTE NOTE: was originally 12 - 6 is what we have been using for our smaller version
#define	MAX_SPACE_COMMAND_SHIPS					192
#define	MAX_SPACE_COMMAND_STATIONARY_ENTITIES	192 // BOYETTE NOTE: was originally 64 - 194 is what we are using now
#define	MAX_SPACE_COMMAND_DYNAMIC_LIGHTS	4
...
// boyette space command begin
	void					UpdateSpaceEntityVisibilities();
	bool					CheckAllNonPlayerSkyPortalsForOccupancy();
	void					SetAllSkyPortalsOccupancyStatusToFalse();
	void					SetAllShipSkyPortalEntitiesToNull();
	void					AssignAllShipsAtCurrentLocToASkyPortalEntity();
	int						GetNumberOfNonPlayerShipSkyPortalEntities();
	int						GetNumberOfNonPlayerShipsAtStarGridPosition(int sgdestx,int sgdesty);

	void					MaybeDisplayStarGridStoryWindow();
	void					UpdateSunEntity();
	bool					stargridPositionsVisitedByPlayer[MAX_STARGRID_X_POSITIONS + 1][MAX_STARGRID_Y_POSITIONS + 1];

	bool					stargridPositionsOffLimitsToShipAI[MAX_STARGRID_X_POSITIONS + 1][MAX_STARGRID_Y_POSITIONS + 1];
	bool					CheckIfStarGridPositionIsOffLimitsToShipAI(int sgposx,int sgposy);
	void					RandomizeStarGridPositionsForCertainSpaceEntities();
	void					GetStarGridPositionsThatAreOffLimitsToShipAI();
	idEntity*				StoryRepositoryEntity;

	idEntity*				SpaceCommandViewscreenCamera;

	idEntity*				space_command_dynamic_lights[MAX_SPACE_COMMAND_DYNAMIC_LIGHTS];
	void					GetSpaceCommandDynamicLights();
	void					UpdateSpaceCommandDynamicLights();

	void					UpdateSpaceCommandViewscreenCamera();

	idVec3					GetSuitableTransporterPositionWithinBounds( idEntity* ent = NULL, const idBounds* target_bounds = NULL );

	// BOYETTE FULLSCREEN GUI BEGIN
	virtual idUserInterface* PlayerGUIOverlay();
	virtual bool			HandleGuiCommandsOnPlayer( const char *cmds );
...
// make sure to change script/icarus_defs.script if you add any channels, or change their order
//
typedef enum {
...
#define	MASK_OPAQUE					(CONTENTS_OPAQUE) // BOYETTE SPACE COMMAND BEGIN: 01/26/16 - (CONTENTS_OPAQUE|CONTENTS_MONSTERCLIP) changed back to original (CONTENTS_OPAQUE) because I found out we can just use a caulk brush or any brush with forceOpaque material global keyword to block AI vision. this was originally just: (CONTENTS_OPAQUE)                 // but for our purposes we need to block the AI vision while still allowing the player to see through it - monsterclip will have to be used for this as it makes the most sense - if only monsters can't pass through something then they cannot see through it. // BOYETTE SPACE COMMAND END
