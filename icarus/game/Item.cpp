...
idItem::idItem() {
	...
	spin_bobbing_height = 0.0f;
	spin_bobbing_height_speed_multiplier = 0.0f;
...
void idItem::Save( idSaveGame *savefile ) const {
	...
	// BOYETTE SAVE BEGIN
	savefile->WriteFloat( spin_bobbing_height );
	savefile->WriteFloat( spin_bobbing_height_speed_multiplier );
	// BOYETTE SAVE END
}
...
void idItem::Restore( idRestoreGame *savefile ) {
	...
	savefile->ReadFloat( spin_bobbing_height );
	savefile->ReadFloat( spin_bobbing_height_speed_multiplier );
	// BOYETTE RESTORE END
}
...
void idItem::Think( void ) {
	if ( thinkFlags & TH_THINK ) {
		if ( spin ) {
			...
			//float scale = 0.005f + entityNumber * 0.00001f; // boyette note: this was the original - 'scale' is not the best word to describe this - this is just so every entity is different - it does not affect the height at all - we created spin_bobbing_height to adjust the height

			float scale = 0.005f + entityNumber * 0.0000001f; // boyette note: changed this so it doesn't go to fast when there are too many entities - 'scale' is not the best word to describe this - this is just so every entity is different - it does not affect the height at all - we created spin_bobbing_height to adjust the height
			
			org = orgOrigin;
			//org.z += 4.0f + cos( ( gameLocal.time + 2000 ) * scale ) * 4.0f; // boyette note: this was the original
			//org.z += spin_bobbing_height + cos( ( (gameLocal.time * spin_bobbing_height_speed_multiplier) + 2000 ) * scale ) * spin_bobbing_height; // boyette note: this is our new version
			org.z += spin_bobbing_height + cos( ( (gameLocal.time * spin_bobbing_height_speed_multiplier) + 2000 + entityNumber ) * scale ) * spin_bobbing_height; // boyette note: this is our newer version
			SetOrigin( org );
		}
	}
...
void idItem::Spawn( void ) {
	...
	if ( spawnArgs.GetBool( "spin" ) || gameLocal.isMultiplayer ) {
		spin = true;
		// boyette space command begin
		spin_bobbing_height = spawnArgs.GetFloat( "spin_bobbing_height", "4.0" );
		spin_bobbing_height_speed_multiplier = spawnArgs.GetFloat( "spin_bobbing_height_speed_multiplier", "1.0" );
		// boyette space command end
		BecomeActive( TH_THINK );
	}
#endif
	...
	// boyette space command begin
	int item_remove_time = spawnArgs.GetInt( "item_remove_time", "0" );
	if ( item_remove_time > 0 ) {
		PostEventMS( &EV_Remove, item_remove_time );
	}
	// boyette space command end
	...
	lastCycle = -1;
	itemShellHandle = -1;
	shellMaterial = declManager->FindMaterial( "textures/materials_used_in_source/item_highlight_shell" );
}
...
bool idItem::Pickup( idPlayer *player ) {
	...
	if ( respawn && !dropped && !no_respawn ) {
		...
		if ( !spawnArgs.GetBool( "inv_carry" ) ) {
			CancelEvents(&EV_Remove); // BOYETTE NOTE: added this for safety with "item_remove_time" - delete this line if it causes problems
...
void idItemTeam::Spawn( void ) {
	...
	if ( team == 0 )
		itemGlow.shader = declManager->FindMaterial( "textures/materials_used_in_source/lights/flag_red" );
	else
		itemGlow.shader = declManager->FindMaterial( "textures/materials_used_in_source/lights/flag_blue" );
	*/
...
idEntity *idMoveableItem::DropItem( const char *classname, const idVec3 &origin, const idMat3 &axis, const idVec3 &velocity, int activateDelay, int removeDelay ) {
	...
	if ( item ) {
		...
		if ( !removeDelay ) {
			//removeDelay = 5 * 60 * 1000; // BOYETTE NOTE: this was the original
			// boyette space command begin
			removeDelay = item->spawnArgs.GetInt( "item_remove_time", "300000" );
			if ( removeDelay < 0 ) {
				removeDelay = 0;
			}
			// boyette space command end
		}
		// always remove a dropped item after 5 minutes in case it dropped to an unreachable location // BOYETTE NOTE: changed this to use the item remove time.
		item->CancelEvents(&EV_Remove); // BOYETTE NOTE: added this for safety with "item_remove_time" - delete this line if it causes problems
...
CLASS_DECLARATION( idItem, sbItem )
END_CLASS
...
================
*/
bool sbItem::GiveToPlayer(idPlayer *player) {
	if ( player ) {
		if ( player->PlayerShip ) {
			//player->PlayerShip->current_currency_reserves = idMath::ClampInt(0,999999, player->PlayerShip->current_currency_reserves + spawnArgs.GetInt("currency_amount","0") );
			//player->PlayerShip->current_materials_reserves = idMath::ClampInt(0,999999, player->PlayerShip->current_materials_reserves + spawnArgs.GetInt("materials_amount","0") );
			int currency_amount_to_give = spawnArgs.GetInt("currency_amount","0");
			int materials_amount_to_give = spawnArgs.GetInt("materials_amount","0");
			player->PlayerShip->GiveThisShipCurrency(currency_amount_to_give);
			player->PlayerShip->GiveThisShipMaterials(materials_amount_to_give);
		}
		gameLocal.Printf( "touched sbItem " + name + "\n");
		return player->GiveItem( this );
	} else {
		return false;
	}
