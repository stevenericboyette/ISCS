...
public:
	...
	// BOYETTE MULTITHREADING BEGIN
	virtual void			AddJobToGameLogicJobList( jobRun_t function, void * data );
	virtual void			SubmitGameLogicJobList( void );
	virtual void			WaitGameLogicJobList( void );
	// BOYETTE MULTITHREADING END
	// BOYETTE BEGIN
	idStr						displayDeviceSupportedResolutions;
	idStr						displayDeviceSupportedRModes;
	// BOYETTE END

	// BOYETTE FULLSCREEN GUI BEGIN
	virtual void			OpenFullScreenGUIOverlay();
	virtual void			CloseFullScreenGUIOverlay();
	virtual void			SyncUpGuiActiveOnMapLoad();
	// BOYETTE FULLSCREEN GUI END

	// BOYETTE KEY INPUT BEGIN // boyette note: this was originally done for multiselect behavoir while holding the shift key
	virtual bool			ShiftKeyIsDown();
	// BOYETTE KEY INPUT END

	// BOYETTE SPACE COMMAND REMOTE RENDER VIEW FIX BEGIN
	virtual void			ReloadMaterialImages( const char *material_name );
	// BOYETTE SPACE COMMAND REMOTE RENDER VIEW FIX END

	// BOYETTE WEIRD TEXTURE THRASHING FIX BEGIN
	virtual void			FlushOpenGL();
...
void Com_ExecMachineSpec_f( const idCmdArgs &args ) {
	if ( com_machineSpec.GetInteger() == 3 ) {
		...
		if ( renderSystem->CheckIfResolutionModeIsSupported(7) ) {
			cvarSystem->SetCVarInteger( "r_mode", 7, CVAR_ARCHIVE );
		} else if ( renderSystem->CheckIfResolutionModeIsSupported(6) ) {
			cvarSystem->SetCVarInteger( "r_mode", 6, CVAR_ARCHIVE );
		} else {
			cvarSystem->SetCVarInteger( "r_mode", 5, CVAR_ARCHIVE ); // original
		}
		...
		if ( renderSystem->CheckIfResolutionModeIsSupported(5) ) {
			cvarSystem->SetCVarInteger( "r_mode", 5, CVAR_ARCHIVE );
		} else {
			cvarSystem->SetCVarInteger( "r_mode", 4, CVAR_ARCHIVE ); // original
		}
	...
	if ( oldCard ) {
		...
		//cvarSystem->SetCVarBool( "g_projectileLights", true, CVAR_ARCHIVE ); // BOYETTE NOTE: commented out 07 12 2016
...
void idCommonLocal::PrintLoadingMessage( const char *msg ) {
	...
	renderSystem->DrawStretchPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, 1, 1, declManager->FindMaterial( "textures/materials_used_in_source/splash_screen" ) );
	int len = strlen( msg );
	renderSystem->DrawSmallStringExt( ( 1280 - len * SMALLCHAR_WIDTH ) / 2, 960 - 70, msg, idVec4( 0.0f, 0.81f, 0.94f, 1.0f ), true, declManager->FindMaterial( "textures/materials_used_in_source/console_font" ) );
...
void idCommonLocal::InitSIMD( void ) {
	idSIMD::InitProcessor( "icarus", com_forceGenericSIMD.GetBool() );
...
void idCommonLocal::Frame( void ) {
	try {
		...
		// Run stats/achievements every frame
...
void idCommonLocal::Init( int argc, const char **argv, const char *cmdline ) {
	...
	// BOYETTE MULTITHREADING BEGIN
	gameLogicJobList = parallelJobManager->AllocJobList( JOBLIST_GAME_LOGIC, JOBLIST_PRIORITY_MEDIUM, 2048, 0, NULL );
	// BOYETTE MULTITHREADING END
...
#ifdef STEAM_BUILD
		common->Printf("STEAM_BUILD INIT\n");
		...
		if ( !SteamAPI_Init() )
		{
			...
			common->FatalError( "Fatal Error: Steam must be running to play this game (SteamAPI_Init() failed).\n" );
		}
		...
		if ( !SteamUser()->BLoggedOn() )
		{
			...
			//common->FatalError( "Fatal Error: Steam user must be logged in to play this game (SteamUser()->BLoggedOn() returned false).\n" );
			common->Warning( "Steam user not logged in. The game is in offline mode.\n" );
		}
		...
		if ( !SteamController()->Init() )
		{
			...
			common->FatalError( "Fatal Error: SteamController()->Init failed.\n" );
		}
...
// BOYETTE MULTITHREADING BEGIN
parallelJobManager->FreeJobList( gameLogicJobList );
gameLogicJobList = NULL;
// BOYETTE MULTITHREADING END

// boyette begin
	displayDeviceSupportedResolutions.Clear();
	displayDeviceSupportedRModes.Clear();
// boyette end
...
/*
=================
idCommonLocal::AddJobToGameLogicJobList
=================
*/
void idCommonLocal::AddJobToGameLogicJobList( jobRun_t function, void * data ) {
	gameLogicJobList->AddJob(function,data);
}
/*
=================
idCommonLocal::SubmitGameLogicJobList
=================
*/
void idCommonLocal::SubmitGameLogicJobList( void ) {
	gameLogicJobList->Submit();
}
/*
=================
idCommonLocal::WaitGameLogicJobList
=================
*/
void idCommonLocal::WaitGameLogicJobList( void ) {
	gameLogicJobList->Wait();
}
...
/*
=================
idCommonLocal::OpenFullScreenGUIOverlay
=================
*/
void idCommonLocal::OpenFullScreenGUIOverlay( void ) {
	if ( game && game->PlayerGUIOverlay() ) {
		session->SetGUI(game->PlayerGUIOverlay(),NULL);
		usercmdGen->WeirdFix(); // this fixes a problem with not being able to close the captain menu when sitting in the captain chair and using 'backspace' after the captai menu was opened without a button press
	}
}
/*
=================
idCommonLocal::CloseFullScreenGUIOverlay
=================
*/
void idCommonLocal::CloseFullScreenGUIOverlay( void ) {
	session->ExitMenu();
}

/*
=================
idCommonLocal::SyncUpGuiActiveOnMapLoad
=================
*/
void idCommonLocal::SyncUpGuiActiveOnMapLoad( void ) {
	if ( game && game->PlayerGUIOverlay() ) {
		session->SetGUIActive( game->PlayerGUIOverlay() );
		//OpenFullScreenGUIOverlay();
		usercmdGen->WeirdFix();
	}
}
// BOYETTE FULLSCREEN GUI END
...
// BOYETTE KEY INPUT BEGIN
/*
=================
idCommonLocal::ShiftKeyIsDown
=================
*/
bool idCommonLocal::ShiftKeyIsDown( void ) {
...
/*
=================
idCommonLocal::ReloadMaterialImages
=================
*/
void idCommonLocal::ReloadMaterialImages( const char *material_name ) {
	session->ReloadMaterialImages(material_name);
}
// BOYETTE SPACE COMMAND REMOTE RENDER VIEW FIX END
...
/*
=================
idCommonLocal::FlushOpenGL
=================
*/
void idCommonLocal::FlushOpenGL( void ) {
	qglFlush();
	qglFinish();
}
// BOYETTE WEIRD TEXTURE THRASHING FIX END
...
/*
=================
idCommonLocal::StoreSteamStats
=================
*/
void idCommonLocal::StoreSteamStats( void ) {
#ifdef STEAM_BUILD
	if ( m_pStatsAndAchievements ) {
		m_pStatsAndAchievements->OnGameStateChange();
	}
#endif
}
// BOYETTE STEAM INTEGRATION END
