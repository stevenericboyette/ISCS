...
float SCR_DrawFPS( float y ) {
	...
	if ( index > FPS_FRAMES ) {
		...
		renderSystem->DrawBigStringExt( 1270 - w, idMath::FtoiFast( y ) + 2, s, colorWhite, true, localConsole.charSetShader);
	}
...
void idConsoleLocal::LoadGraphics() {
	charSetShader = declManager->FindMaterial( "textures/materials_used_in_source/console_font" );
	whiteShader = declManager->FindMaterial( "_white" );
	consoleShader = declManager->FindMaterial( "textures/materials_used_in_source/console_background" );
}
