...
void idSessionLocal::Clear() {
	...
	// BOYETTE ADJUSTED SCREEN WIPE BEGIN
	restartingCurrentMap = false;
	// BOYETTE ADJUSTED SCREEN WIPE END
...
void idSessionLocal::Stop() {
	...
	// BOYETTE BEGIN
	cvarSystem->SetCVarBool( "g_inIronManMode", false );
	// BOYETTE END
}
...
void idSessionLocal::StartWipe( const char *_wipeMaterial, bool hold ) {
	...
	renderSystem->CropRenderSize( 1280, 960, true );
...
void idSessionLocal::LoadLoadingGui( const char *mapName ) {
	...
	guiLoading->SetStateString( "map_loading_status_text", "Beginning Load..." );
}
...
void idSessionLocal::ExecuteMapChange( bool noFadeWipe ) {
	...
	// BOYETTE SPACE COMMAND BEGIN // Here is as good a place as any
	cvarSystem->SetCVarBool( "g_stopTime", false );
	cvarSystem->SetCVarBool( "g_stopTimeForceFrameNumUpdateDuring", false );
	cvarSystem->SetCVarBool( "g_stopTimeForceRenderViewUpdateDuring", false );
	...
	if ( !noFadeWipe ) {
		...
		if ( currentMapName.IsEmpty() ) {
			StartWipe( "textures/materials_used_in_source/screen_wipe_to_black", true );
		} else if ( restartingCurrentMap ) {
			StartWipe( "textures/materials_used_in_source/screen_wipe_to_black_with_reloading_text", true );
		} else if ( !mapSpawned || (loadingSaveGame && savegameFile) ) {
			StartWipe( "textures/materials_used_in_source/screen_wipe_to_black_with_loading_text", true );
		} else if ( fullMapName == currentMapName ) {
			StartWipe( "textures/materials_used_in_source/screen_wipe_to_black_with_reloading_text", true );
		} else {
			StartWipe( "textures/materials_used_in_source/screen_wipe_to_black_with_unloading_text", true );
		}
		// BOYETTE ADJUSTED SCREEN WIPE END
	...
	guiLoading->SetStateString( "map_loading_status_text", "Loading Level Geometry..." );
	...
	guiLoading->SetStateString( "map_loading_status_text", "Loading and Spawning Map Entities..." );
	...
	if ( !idAsyncNetwork::IsActive() && !loadingSaveGame ) {
		// spawn players
		for ( i = 0; i < numClients; i++ ) {
			guiLoading->SetStateString( "map_loading_status_text", "Spawning Player..." );
			game->SpawnPlayer( i );
		}
	}
	...
	if ( !reloadingSameMap ) {
		guiLoading->SetStateString( "map_loading_status_text", "Loading Media: Models and Images..." );
		renderSystem->EndLevelLoad();
		guiLoading->SetStateString( "map_loading_status_text", "Loading Media: Sounds..." );
		soundSystem->EndLevelLoad( mapString.c_str() );
		guiLoading->SetStateString( "map_loading_status_text", "Loading Media: Declarations..." );
		declManager->EndLevelLoad();
		SetBytesNeededForMapLoad( mapString.c_str(), fileSystem->GetReadCount() );
	}
	guiLoading->SetStateString( "map_loading_status_text", "Loading Media: User Interfaces..." );
	uiManager->EndLevelLoad();

	if ( !idAsyncNetwork::IsActive() && !loadingSaveGame ) {
		// run a few frames to allow everything to settle
		guiLoading->SetStateString( "map_loading_status_text", "Running A Few Frames To Let Things Settle..." );
		for ( i = 0; i < 10; i++ ) {
	...
	guiLoading->SetStateString( "map_loading_status_text", "Generating Pre-Computed Light And Shadow Interactions..." );
	...
	if ( guiLoading && bytesNeededForMapLoad ) {
		...
		// BOYETTE END
		while ( pct < 1.0f ) {
			guiLoading->SetStateString( "map_loading_status_text", "Pretending to do stuff..." );
	...
	// capture the current screen and start a wipe
	StartWipe( "textures/materials_used_in_source/screen_wipe_to_clear" );
...
void SaveGame_f( const idCmdArgs &args ) {
	if ( args.Argc() < 2 || idStr::Icmp( args.Argv(1), "quick" ) == 0 || idStr::Icmp( args.Argv(1), "IRONMAN" ) == 0 ) {
		if ( idStr::Icmp( args.Argv(1), "IRONMAN" ) == 0 ) {
			idStr saveName = common->GetLanguageDict()->GetString( "^4IRONMAN^0" );
...
===============
*/
void RemoveSaveGame_f( const idCmdArgs &args ) {
	if ( args.Argc() == 2 ) {
		// setup up filenames and paths
		idStr gameFile, previewFile, descriptionFile, mapName;

		gameFile = args.Argv(1);
		sessLocal.ScrubSaveGameFileName( gameFile );
		...
		fileSystem->RemoveFile( descriptionFile );
		common->Printf( "Removed %s\n", args.Argv(1) );
	}
}
...
void idSessionLocal::TakeNotes( const char *p, bool extended ) {
	...
	if ( extended ) {
		guiTakeNotes = uiManager->FindGui( "guis/take_notes_2.gui", true, false, true );
		...
		guiTakeNotes = uiManager->FindGui( "guis/take_notes_1.gui", true, false, true );
	}
...
bool idSessionLocal::LoadGame( const char *saveName ) { 
	...
	// BOYETTE BEGIN: we don't use persistentPlayerInfo to give the player things if the savegame version doesn't match
	if ( savegameVersion != SAVEGAME_VERSION && SAVEGAME_VERSION > 17 ) {
		for ( i = 0; i < MAX_ASYNC_CLIENTS; i++ ) {
			mapSpawnData.persistentPlayerInfo[i].Clear();
		}
	}
	...
	common->SyncUpGuiActiveOnMapLoad();
...
void	idSessionLocal::DrawWipeModel() {
	...
	renderSystem->DrawStretchPic( 0, 0, 1280, 960, 0, 0, 1, 1, wipeMaterial );
}
...
void idSessionLocal::DrawCmdGraph() {
	...
	renderSystem->DrawStretchPic( 0, 960-ANGLE_GRAPH_HEIGHT, MAX_BUFFERED_USERCMD*ANGLE_GRAPH_STRETCH, ANGLE_GRAPH_HEIGHT, 0, 0, 1, 1, whiteMaterial );
	...
	for ( int i = 0 ; i < MAX_BUFFERED_USERCMD-4 ; i++ ) {
		...
		renderSystem->DrawStretchPic( i* ANGLE_GRAPH_STRETCH, 960-h, 1, h, 0, 0, 1, 1, whiteMaterial );
	}
}
...
void idSessionLocal::PacifierUpdate() {
	...
	if ( guiLoading && bytesNeededForMapLoad ) {
		...
		// BOYETTE BEGIN // BOYETTE NOTE: this is because we load more bytes than the max value of int - so int readCount goes negative - we should make readCount and bytesNeededForMapLoad unsigned ints for twice the value or floats would be the best solution probably - floats could handle any amount of bytes we would ever load
		 // boyette note: this is a temp solution until we can convert these readCount and bytesNeededForMapLoad to actually just be floats - which should be possible and allow any number of bytes we would ever load
		 // boyette note: at least this temp solution - n handles up until 2,147,483,647 * 2 = 4,294,967,294 (or maybe 4,294,967,295) - it will still overflow because bytesNeededForMapLoad gets capped at 2,147,483,647 somewhere else
		int max_int_size = (std::numeric_limits<int>::max)(); //2,147,483,647
		unsigned int max_unsigned_int_size = (std::numeric_limits<unsigned int>::max)(); //4,294,967,295
		if ( bytesNeededForMapLoad >= max_int_size ) {
			n = (unsigned int)fileSystem->GetReadCount();
			pct = ( n / (float)max_unsigned_int_size );
		}
		...
		if ( pct < 0.0f ) {
			n = (unsigned int)fileSystem->GetReadCount();
			pct = ( n / (unsigned int)bytesNeededForMapLoad ); // this (unsigned int) cast doesn't do anything here because bytesNeededForMapLoad gets capped at 2,147,483,647 somewhere else
			//if ( pct < 0.0f || pct >= 1.0f ) {
			//	pct = 0.9f;
			//}
		}
		*/
		//common->Printf( "Current GetReadCount() = %u\n", fileSystem->GetReadCount() );
		//common->Printf( "pct = %f\n", pct );
		// BOYETTE END
		guiLoading->SetStateFloat( "map_loading", pct );
		guiLoading->StateChanged( com_frameTime );
...
void idSessionLocal::Draw() {
	...
	if ( insideExecuteMapChange ) {
		...
		renderSystem->DrawStretchPic( 0, 0, 1280, 960, 0, 0, 1, 1, declManager->FindMaterial( "_white" ) );
		...
		if ( guiActive && game && guiActive == game->PlayerGUIOverlay() ) {
			//guiActive->StateChanged( com_frameTime, true );
			//guiActive->Redraw( com_frameTime );
			guiActive->Redraw( game->GetTimeGroupTime(1) );
		}
...
void idSessionLocal::Frame() {
	...
	if ( guiActive && guiActive != game->PlayerGUIOverlay() ) {
...
void idSessionLocal::Init() {
	...
	cmdSystem->AddCommand( "removeSaveGame", RemoveSaveGame_f, CMD_FL_SYSTEM|CMD_FL_CHEAT, "removes a save game" );
#endif
	...
	guiMsg = uiManager->FindGui( "guis/drm_msg.gui", true, false, true );
	guiTakeNotes = uiManager->FindGui( "guis/take_notes_1.gui", true, false, true );
...
bool idSessionLocal::CDKeysAreValid( bool strict ) {
	...
	*/ // BOYETTE SPACE COMMAND END
	return true; // BOYETTE SPACE COMMAND NOTE - this was added to the function
}
