...
const idRegEntry idWindow::RegisterVars[] = {
	...
	{ "background_parm11", idRegister::FLOAT },
	// BOYETTE NOTE: added so we can set screen resolution choiceDef with "gui::display_device_supported_r_modes"
	{ "values", idRegister::STRING },
	// BOYETTE NOTE: added so we can set screen resolution choiceDef
	{ "tooltip_text", idRegister::STRING }
};
...
void idWindow::CommonInit() {
	...
	xOffset = yOffset = 0.0;
	cursor = 0;
	forceAspectWidth = 1280;
	forceAspectHeight = 960;
	matScalex = 1;
	matScaley = 1;
	borderSize = 0;
	noTime = false;
	...
	textShadow = 0;
	hover = false;
	background_parm11 = 0.0f;
	has_custom_parm11 = false;
	...
	hideCursor = false;
	allow_multiselect_box_events = true;
}
...
// boyette space command begin // if the cursor is set to some kind of targeting reticule(i.e. greater than 1) then keep it that way until we change it back. Once we change it back - return the cursor to its normal behavior.
		if ( dc->GetCursorNum() < 2 ) {
...
// boyette space command end
		...
		if (event->evType == SE_KEY) {
			...
			if (event->evValue == K_MOUSE1) {
				...
				if (event->evValue2 && !actionDownRun) {
					//BOYETTE NOTE TODO: Need to somehow clear the previous multiselection. This might do it but should upgrade it in the future.
					/*
					if ( gui->GetPendingCmd().Length() ) {
						gui->GetPendingCmd() += " ; ";
						gui->GetPendingCmd() += "clear_crew_multiselection"; // we still need to implement this command.
					} else {
						gui->GetPendingCmd() += "clear_crew_multiselection"; // we still need to implement this command.
					}
					*/
					...
					if ( gui->multiselect_box_active == false && allow_multiselect_box_events ) {
						gui->multiselect_box_active = true;
						gui->multiselect_box_origin_x = gui->CursorX();
						gui->multiselect_box_origin_y = gui->CursorY();
					}
					// boyette space command end
					actionDownRun = RunScript( ON_ACTION );
				} else if (!actionUpRun) {
					// boyette space command begin
					if ( gui->multiselect_box_active == true && allow_multiselect_box_events ) {
						gui->multiselect_box_active = false;
						gui->EndMultiselectBox();
					}
					// boyette space command end
	...
	/*
	if ( test ) {
		common->Printf( "%s has an event type of: %i.\n", GetName(), event->evType );
		common->Printf( "%s has an event value of: %i.\n", GetName(), event->evValue );
		common->Printf( "%s has an event value2 of: %i.\n", GetName(), event->evValue2 );
	}
	*/
	// BOYETTE SPACE COMMAND END
...
void idWindow::Time() {
	...
	if ( gui->Active() || gui->handle_time_commands_when_not_active ) {
...
void idWindow::DrawBackground(const idRectangle &drawRect) {
	...
	if ( background && matColor.w() ) {
		...
		if ( has_custom_parm11 ) {
			background->SetParm11ExpressionRegister(background_parm11);
		}
...
void idWindow::Redraw(float x, float y) {
	...
	if (gui_edit.GetBool()  || (flags & WIN_DESKTOP && !( flags & WIN_NOCURSOR )  && !hideCursor && (gui->Active() || ( flags & WIN_MENUGUI ) ))) {
		...
		gui->DrawCursor();
		// boyette space command begin - BOYETTE NOTE TODO: also need a boolean on the desktop to specify whether or not multiselect is enabled on this gui.
		if ( gui && gui->multiselect_box_active && allow_multiselect_box_events && gui->GetDesktop()->flags & WIN_ALLOWMULTISELECT ) {
			gui->BeginMultiselectBox();
		}
		// boyette space command end
	}
...
int idWindow::GetWinVarOffset( idWinVar *wv, drawWin_t* owner) {
	...
	if ( wv == &background_parm11 ) {
		ret = (int)&( ( idWindow * ) 0 )->background_parm11;
	}
...
idWinVar *idWindow::GetWinVarByName(const char *_name, bool fixup, drawWin_t** owner) {
	...
	if (idStr::Icmp(_name, "background_parm11") == 0) {
		has_custom_parm11 = true;
		retVar = &background_parm11;
	}
	if (idStr::Icmp(_name, "tooltip_text") == 0) {
		retVar = &tooltip_text;
	}
...
// BOYETTE SPACE COMMAND BEGIN
	if (idStr::Icmp(_name, "allowmultiselect") == 0) {
		if ( src->ParseBool() ) {
			flags |= WIN_ALLOWMULTISELECT;
		}
		return true;
	}
// BOYETTE SPACE COMMAND END
...
void idWindow::SetInitialState(const char *_name) {
	...
	matScalex = 1.0;
	matScaley = 1.0;
	forceAspectWidth = 1280.0;
	forceAspectHeight = 960.0;
	noTime = false;
	visible = true;
	flags = 0;
}
...
void idWindow::WriteToSaveGame( idFile *savefile ) {
	...
	// BOYETTE SAVE BEGIN
	savefile->Write( &allow_multiselect_box_events, sizeof( allow_multiselect_box_events ) );
	savefile->Write( &has_custom_parm11, sizeof( has_custom_parm11 ) );
	// BOYETTE SAVE END
	...
	background_parm11.WriteToSaveGame( savefile );
	tooltip_text.WriteToSaveGame( savefile );
...
void idWindow::ReadFromSaveGame( idFile *savefile ) {
	...
	// BOYETTE SAVE BEGIN
	savefile->Read( &allow_multiselect_box_events, sizeof( allow_multiselect_box_events ) );
	savefile->Read( &has_custom_parm11, sizeof( has_custom_parm11 ) );
	// BOYETTE SAVE END
	...
	background_parm11.ReadFromSaveGame( savefile );
	tooltip_text.ReadFromSaveGame( savefile );
	...
	for ( i = 0; i < c; i++ ) {
		...
		if ( dw && ( dw->win || dw->simp ) ){
			if ( dw->win ) {
				...
				} else if ( transitions[i].offset == (int)&( ( idWindow * ) 0 )->background_parm11 ) {
					transitions[i].data = &dw->win->background_parm11;
				}
			} else {
				...
				} else if ( transitions[i].offset == (int)&( ( idSimpleWindow * ) 0 )->background_parm11 ) {
					transitions[i].data = &dw->simp->background_parm11;
				}
			}
		}
...
bool idWindow::IsSimple() {
	...
	// BOYETTE NOTE BEGIN: simple windows don't get added to drawWindows or children lists - so GetChildCount and GetChild won't work - added this to fix it
...
void idWindow::SetDefaults ( void ) {	
	forceAspectWidth = 1280.0f;
	forceAspectHeight = 960.0f;
	matScalex = 1;
	matScaley = 1;
	borderSize = 0;
	noTime = false;
