...
idSimpleWindow::idSimpleWindow(idWindow *win) {
	...
	background_parm11 = win->background_parm11;
	tooltip_text = win->tooltip_text;
...
// 
	...
	if (parent) {
		...
		if (background_parm11.NeedsUpdate()) {
			parent->AddUpdateVar(&background_parm11);
		}
		if (tooltip_text.NeedsUpdate()) {
			parent->AddUpdateVar(&tooltip_text);
		}
	}
}
...
int idSimpleWindow::GetWinVarOffset( idWinVar *wv, drawWin_t* owner) {
	...
	if ( wv == &background_parm11 ) {
		ret = (int)&( ( idSimpleWindow * ) 0 )->background_parm11;
	}
